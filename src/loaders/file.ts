...
export const fileParsers: Record<ParserTypesType, (data: string) => configKeys> = {
  ...
  ENV: () => { throw new Error("Parser not implemented yet"); }, // TODO implement
  FROM_EXT: () => { throw new Error("Cannot use FROM_EXT as a parsing type"); }, // TODO proper error
}
...
export function populateLoaderFromFile(loader: configLoader, path: string, type: ParserTypesType = ParserTypes.FROM_EXT) {
  if (type === ParserTypes.FROM_EXT) {
    const extType = parserMap[getExtension(path)];
    if (!extType)
      throw new Error("invalid extension, cannot load file"); // TODO proper error
    type = extType;
  }
  if (!Object.values(ParserTypes).includes(type))
    throw new Error("invalid parser type, cannot load file") // TODO proper error
  loader.files.push({
    path,
    type,
  })
